...
def test_update_user():
    with my_test_environment():
        ...
        with open('./events/event-put-user.json', 'r') as f:
            apigw_event = json.load(f)
        # Fix the event for PUT operation
        apigw_event['resource'] = '/users/{userid}'
        apigw_event['pathParameters'] = {'userid': UUID_MOCK_VALUE_JOHN}
        ret = users.lambda_handler(apigw_event, '')
        assert ret['statusCode'] == 200
        data = json.loads(ret['body'])
...
def test_put_user_invalid_json():
    with my_test_environment():
        from src.api import users
        apigw_event = {
            'httpMethod': 'PUT',
...
@patch.dict(os.environ, {'USERS_TABLE': 'NonExistentTable', 'AWS_XRAY_CONTEXT_MISSING': 'LOG_ERROR'})
...
def test_delete_nonexistent_user():
    with my_test_environment():
        ...
        apigw_event = {
            'httpMethod': 'DELETE',
            'resource': '/users/{userid}',
            'pathParameters': {'userid': 'nonexistent-id'}
        }
        ret = users.lambda_handler(apigw_event, '')
        assert ret['statusCode'] == 200
        assert json.loads(ret['body']) == {}
